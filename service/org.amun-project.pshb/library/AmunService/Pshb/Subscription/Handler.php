 ...
 */
class AmunService_Pshb_Subscription_Handler extends Amun_Data_HandlerAbstract
{
	public function create(PSX_Data_RecordInterface $record)
	{
		if($record->hasFields('topic'))
		{
			...
			$record->status = AmunService_Pshb_Subscription_Record::PENDING;
	...
	protected function request($url)
	{
		...
		{
			$callback    = new PSX_Url($this->config['psx_url'] . '/' . $this->config['psx_dispatch'] . 'api/service/pshb/callback');
			$topic       = $url;
			...
			// insert
			$subscription = new AmunService_Pshb_Subscription_Record($this->table);
