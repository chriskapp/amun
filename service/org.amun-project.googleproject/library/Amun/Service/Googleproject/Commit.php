<?php
/*
 *  $Id: Commit.php 638 2012-05-01 20:50:26Z k42b3.x@googlemail.com $
 *
 * amun
...
class Amun_Service_Googleproject_Commit extends Amun_Data_RecordAbstract
{
	...
	public function setTimestamp($timestamp)
	{
		...
		if(!$this->_validate->hasError())
		{
			$date = new DateTime(date(PSX_Time::SQL, $timestamp), new DateTimeZone('UTC'));
			$date->setTimezone($this->_registry['core.default_timezone']);

			$this->commitDate = $date->format(PSX_Time::SQL);
		}
		else
		{
			throw new PSX_Data_Exception($this->_validate->getLastError());
