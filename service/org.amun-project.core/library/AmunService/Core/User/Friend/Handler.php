 ...
 */
class AmunService_Core_User_Friend_Handler extends Amun_Data_HandlerAbstract
{
...
SELECT
	...
	FROM {$this->registry['table.core_system_host']} `host`
...
SQL;
		...
		if(!empty($row))
		{
			...
			$friendId = $this->sql->select($this->registry['table.core_user_account'], array('id'), $con, PSX_Sql::SELECT_FIELD);
...
SELECT
	...
	FROM {$this->registry['table.core_user_account']} `account`
		...
		INNER JOIN {$this->registry['table.core_system_host']} `host`
