...
/**
 * Amun_User_Friend_Group_Handler
 *
 ...
 */
class AmunService_Core_User_Friend_Group_Handler extends Amun_Data_HandlerAbstract
{
	public function create(PSX_Data_RecordInterface $record)
	{
		if($record->hasFields('title'))
		{
			...
			{
				throw new PSX_Data_Exception('Max count of groups reached');
			}
	...
	public function delete(PSX_Data_RecordInterface $record)
	{
		if($record->hasFields('id'))
		{
			...
			$this->sql->update($this->registry['table.core_user_friend'], array('groupId' => 0), $con);
