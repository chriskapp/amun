<?php
/*
 *  $Id: Handler.php 759 2012-07-01 12:50:19Z k42b3.x@googlemail.com $
 *
 * amun
 ...
 */
class Amun_Service_Plugin_Release_Handler extends Amun_Data_HandlerAbstract
{
		...
		if($record->hasFields('pluginId', 'status', 'version', 'href'))
		{
			...
			if(!$this->hasApproval($record))
			{
				...
				$this->sql->update($this->registry['table.service_plugin'], array(
					...
					'releaseDate' => $date->format(PSX_Time::SQL),
