...
class edit extends Amun_Module_ApplicationAbstract
{
	...
	{
		if($this->user->hasRight('service_tracker_edit'))
		{
			// form url
			$id = $this->get->id('integer');

			$url = $this->config['psx_url'] . '/' . $this->config['psx_dispatch'] . 'api/service/tracker/form?format=json&method=update&id=' . $id;
			...
			// add path
			$this->path->add($tracker->title, $this->page->url . '/view?id=' . $tracker->id);
...
amun.services.tracker.loadForm("form", "{$url}");
</script>
HTML;
		}
		else
