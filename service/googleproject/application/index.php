...
class index extends Amun_Module_ApplicationAbstract
{
	...
	{
		if($this->user->hasRight('service_googleproject_view'))
		{
			// load page
			...
			$options->add('service_googleproject_edit', 'Edit', $this->page->url . '/edit' . (!empty($this->project) ? '?id=' . $this->project->id : ''));
			...
			$this->htmlContent->add(Amun_Html_Content::META, PSX_Data_Writer_Atom::link($this->page->title, $this->config['psx_url'] . '/' . $this->config['psx_dispatch'] . 'api/service/googleproject/commit?format=atom&filterBy=pageId&filterOp=equals&filterValue=' . $this->page->id));
	...
	{
		return Amun_Sql_Table_Registry::get('Service_Googleproject')
	...
	{
		$select = Amun_Sql_Table_Registry::get('Service_Googleproject_Commit')
