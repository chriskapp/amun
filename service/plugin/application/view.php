...
class view extends Amun_Module_ApplicationAbstract
{
	...
	public function onLoad()
	{
		if($this->user->hasRight('service_plugin_view'))
		{
			...
			// form url
			$url = $this->config['psx_url'] . '/' . $this->config['psx_dispatch'] . 'api/service/comment/form?format=json&method=create&pageId=' . $this->page->id . '&refId=' . $this->pluginId;
	...
	private function getPlugin()
	{
		...
		// query
		$result = Amun_Sql_Table_Registry::get('Service_Plugin')
	...
	{
		$result = Amun_Sql_Table_Registry::get('Service_Plugin_Maintainer')
	...
	{
		$result = Amun_Sql_Table_Registry::get('Service_Plugin_Release')
