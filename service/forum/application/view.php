...
class view extends Amun_Module_ApplicationAbstract
{
	...
	public function onLoad()
	{
		if($this->user->hasRight('service_forum_view'))
		{
			...
			// options
			$url     = $this->config['psx_url'] . '/' . $this->config['psx_dispatch'] . 'api/service/forum';
			$options = new Amun_Option(__CLASS__, $this->registry, $this->user, $this->page);

			if($recordForum->isSticky())
			{
				$options->add('service_forum_sticky', 'Unstick', 'javascript:amun.services.forum.setSticky(' . $this->forumId . ',0,\'' . $url . '\',this)');
			}
			else
			{
				$options->add('service_forum_sticky', 'Sticky', 'javascript:amun.services.forum.setSticky(' . $this->forumId . ',1,\'' . $url . '\',this)');
			}

			if($recordForum->isClosed())
			{
				$options->add('service_forum_close', 'Open', 'javascript:amun.services.forum.setClosed(' . $this->forumId . ',0,\'' . $url . '\',this)');
			}
			else
			{
				$options->add('service_forum_close', 'Close', 'javascript:amun.services.forum.setClosed(' . $this->forumId . ',1,\'' . $url . '\',this)');
			}

			$options->add('service_forum_edit', 'Edit', $this->page->url . '/edit?id=' . $this->forumId);
			$options->load(array($this->page, $recordForum));
