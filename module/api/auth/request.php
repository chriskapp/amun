...
SQL;
	...
	protected function getResponse(PSX_Oauth_Provider_Data_Consumer $consumer, PSX_Oauth_Provider_Data_Request $request)
	{
		...
		$count    = $this->sql->count($this->registry['table.system_api_request'], $con);

		if($count >= $maxCount)
		{
			$conDelete = new PSX_Sql_Condition();
			$result    = $this->sql->select($this->registry['table.system_api_request'], array('id', 'expire', 'date'), $con, PSX_Sql::SELECT_ALL);
			...
			{
				$this->sql->delete($this->registry['table.system_api_request'], $conDelete);
			}
