 ...
 */
class Amun_Content_Gadget_Form extends Amun_Data_FormAbstract
{
	...
	private function listGadget()
	{
		...
		// service gadgets
		$result = $this->sql->getAll('SELECT id, name FROM ' . $this->registry['table.content_service'] . ' WHERE status = ? ORDER BY name ASC', array(Amun_Content_Service::NORMAL));
