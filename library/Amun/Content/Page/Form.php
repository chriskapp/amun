 ...
 */
class Amun_Content_Page_Form extends Amun_Data_FormAbstract
{
	...
	{
		$service = array();
		$result  = $this->sql->getAll('SELECT id, name FROM ' . $this->registry['table.content_service'] . ' WHERE status = ' . intval(Amun_Content_Service::NORMAL) . ' ORDER BY name ASC');
