 ...
 */
class Amun_Content_Service_Handler extends Amun_Data_HandlerAbstract
{
	...
	public function delete(PSX_Data_RecordInterface $record)
	{
		if($record->hasFields('id'))
		{
			...
			// delete options
			$this->sql->delete($this->registry['table.content_service_option'], $con);
	...
	private function parsePermissions(Amun_Content_Service $record)
	{
				...
				for($i = 0; $i < $perms->length; $i++)
				{
					...
					if($perm->nodeName == 'perm')
					{
						...
						if(!empty($name) && !empty($desc))
						{
							...
							$this->sql->insert($this->registry['table.amun_user_right'], array(
								'name'        => $name,
								'description' => $desc,
							));
	...
	{
		// tables
		$result = $this->sql->getAll('SELECT name, value FROM ' . $this->registry['table.system_registry'] . ' WHERE name LIKE "table.%"');
