 ...
 */
class Amun_User_Friend_Handler extends Amun_Data_HandlerAbstract
{
...
SELECT
	...
	FROM {$this->registry['table.system_host']} `host`
...
SELECT
	...
	FROM {$this->registry['table.user_account']} `account`
		...
		INNER JOIN {$this->registry['table.system_host']} `host`
