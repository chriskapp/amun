 ...
 */
class Amun_User_Account_Form extends Amun_Data_FormAbstract
{
	...
	{
		$host   = array();
		$result = $this->sql->getAll('SELECT id, name FROM ' . $this->registry['table.system_host']);
	...
	{
		$country = array();
		$result  = $this->sql->getAll('SELECT id, title FROM ' . $this->registry['table.system_country']);
