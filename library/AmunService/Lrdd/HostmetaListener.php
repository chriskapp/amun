...
class HostmetaListener extends ListenerAbstract
{
	public function notify(XMLWriter $writer)
	{
		...
		$writer->writeAttribute('rel', 'lrdd');
		$writer->writeAttribute('type', 'application/xrd+xml');
		$writer->writeAttribute('template', $this->config['psx_url'] . '/' . $this->config['psx_dispatch'] . 'api/lrdd?uri={uri}');
		$writer->endElement();
	}
}
