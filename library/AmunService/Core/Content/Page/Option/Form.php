 ...
 */
class AmunService_Core_Content_Page_Option_Form extends Amun_Data_FormAbstract
{
	...
	{
		$record = Amun_Sql_Table_Registry::get('Core_Content_Page_Option')->getRecord($id);
...
SELECT
	...
	FROM {$this->registry['table.core_content_service']} `service`
		...
		INNER JOIN {$this->registry['table.core_content_service_option']} `option`
...
SQL;
	...
	{
		$right  = array();
		$result = $this->sql->getAll('SELECT id, name FROM ' . $this->registry['table.core_user_right'] . ' ORDER BY name ASC');
