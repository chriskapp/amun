...
/**
 * AmunService_Core_System_Api_Handler
 *
 ...
 */
class AmunService_Core_System_Api_Handler extends Amun_Data_HandlerAbstract
{
	public function create(PSX_Data_RecordInterface $record)
	{
		if($record->hasFields('status', 'name', 'email', 'url', 'title', 'description'))
		{
			$record->consumerKey    = Amun_Security::generateToken();
			$record->consumerSecret = Amun_Security::generateToken();
